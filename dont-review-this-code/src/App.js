...
function App() {
  ...
  const fetchGet = useCallback(async () => {
    try {
      ...
      const responsePostsError = responsePosts.status === "rejected";
      setData(
        responseUsers.value.data.map((user) => {
          const posts = responsePostsError
            ? []
            : responsePosts.value.data
            ...
            <div key={id}>
              ...
              <div>
                ...
                <ul>
                  {posts.length === 0 ? (
                    <div>
                      <h2>Error get posts</h2>
                      <button onClick={() => handleFetchAgain()}>
                        try again
                      </button>
                    </div>
                  ) : (
