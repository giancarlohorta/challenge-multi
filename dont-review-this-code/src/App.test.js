...
test("dont render elements and render message error", async () => {
  mockRequest(400, 200);
  render(<App />);
  const error = await screen.findByText("Error get data");
  expect(error).toBeInTheDocument();
});
test("render elements about users and render message error about posts", async () => {
  mockRequest(200, 400);
  render(<App />);
  const company = await screen.findByText("Romaguera-Crona");
  expect(company).toBeInTheDocument();
  const error = await screen.findByText("Error get posts");
  expect(error).toBeInTheDocument();
});
